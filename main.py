...
def find_possible_vectorized(guess, reported_sim, words_to_consider):
    ...
    candidates = np.where(np.abs(cosines - reported_sim) <= 0.05)[0]
...
def find_possible(guess, reported_sim, words_to_consider):
    ...
    for w in tqdm(words_to_consider):
        if abs(similarity(guess, w) - reported_sim) <= 0.05:
            ans.append(w)
    return ans
...
def scale_similarity(new_sim):
    """
    Scales the new similarity value from the range -1000 to 1000 to the range -100.00 to 100.00.

    :param new_sim: The new similarity value in the range -1000 to 1000.
    :return: The scaled similarity value in the range -100.00 to 100.00.
    """
    # Normalize to range -1 to 1
    normalized = new_sim / 1000
    # Scale to range -100.00 to 100.00
        ...
        while True:
            ...
            if ", " in data:
                try:
                    ...
                    reported_sim = scale_similarity(int(reported_sim))  # Use the scaling function
                    break
